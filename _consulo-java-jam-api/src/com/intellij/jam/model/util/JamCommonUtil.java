...
public class JamCommonUtil {
  ...
  public static Object computeMemberValue(@Nullable final PsiElement value) {
    ...
    try {
      return JavaPsiFacade.getInstance(value.getProject()).getConstantEvaluationHelper()
        .computeConstantExpression(value, false);
    }
  ...
  public static boolean processSuperClassList(@Nullable final PsiClass firstClass,
                                              @NotNull final Collection<PsiClass> supers,
                                              final Processor<PsiClass> processor) {
    for (PsiClass curClass = firstClass;
         curClass != null && !CommonClassNames.JAVA_LANG_OBJECT.equals(curClass.getQualifiedName()) && !supers.contains(curClass);
  ...
  public static boolean isPlainJavaFile(final PsiElement element) {
    return isKindOfJavaFile(element) &&
           ((PsiFile)element).getFileType() == JavaFileType.INSTANCE; // prevent jsp processing
  }
  ...
  public static Module[] getAllModuleDependencies(@NotNull final Module module) {
    ...
    if (value == null) {
      module.putUserData(MODULE_DEPENDENCIES, value =
        CachedValuesManager.getManager(module.getProject()).createCachedValue(new CachedValueProvider<Module[]>() {
  ...
  public static Collection<PsiClass> getAnnotationTypesWithChildren(final Module module,
    ...
    return !processObjectArrayValue(psiAnnotation, "value", new Processor<PsiAnnotationMemberValue>() {
      ...
      public boolean process(PsiAnnotationMemberValue value) {
        ...
        for (ElementType type : elementTypes) {
          if (type.equals(obj)) return false;
        }
        return true;
      }
    });
  ...
  public static <M extends PsiModifierListOwner, V extends JamElement> List<V> getElementsIncludingSingle(final M member, final JamAnnotationMeta multiMeta,
  ...
  public static Object getModelObject(PsiElement element) {
    ...
    return target instanceof CommonModelTarget ? ((CommonModelTarget)target).getCommonElement() :
      target instanceof JamPomTarget ? ((JamPomTarget)target).getJamElement() :
      target instanceof DomTarget ? ((DomTarget)target).getDomElement() : null;
  }
  ...
  public static DeleteProvider createDeleteProvider(final AbstractTreeBuilder builder) {
    ...
    return new DeleteProvider() {
      ...
      public boolean canDeleteElement(@NotNull DataContext dataContext) {
        for (DeleteProvider provider : toRun) {
          if (!provider.canDeleteElement(dataContext)) return false;
        }
        return true;
      }
    };
  ...
  public static <T extends JamElement> CachedValue<List<T>> createClassCachedValue(final Project project,
                                                                                   final Factory<GlobalSearchScope> scope,
  ...
  public static <T extends PsiModifierListOwner> void findAnnotatedElements(Class<T> elementClass, String annotationClass,
  ...
  public static boolean processObjectArrayValue(PsiAnnotation annotation, String attributeName, Processor<PsiAnnotationMemberValue> processor) {
    if (annotation != null) {
      ...
      if (memberValue instanceof PsiArrayInitializerMemberValue) {
        ...
        if (!processor.process(memberValue)) return false;
      }
    }
    return true;
  }
