...
public class SpringElInjector implements MultiHostInjector {
  ...
  public void getLanguagesToInject(@NotNull MultiHostRegistrar registrar, @NotNull PsiElement host) {
    ...
    if (!(file instanceof XmlFile) || !SpringManager.getInstance(host.getProject()).isSpringBeans((XmlFile)file)) {
      return;
    }
    List<TextRange> ranges = ELResolveUtil.getELTextRanges(host, "#{", "}");
    if (!ranges.isEmpty()) {
      for (TextRange textRange : ranges) {
        registrar.startInjecting(ELLanguage.INSTANCE)
          .addPlace(null, null, (PsiLanguageInjectionHost)host, textRange)
          .doneInjecting();
      }
      host.putUserData(ELContextProvider.ourContextProviderKey, new SpringElContextProvider((XmlAttributeValue)host));
